...
class Game < ApplicationRecord
  attr_reader :deck, :players, :turn, :books_display_name, :books_display_number, :hash_to_return,
              :winning_books, :started
  ...
  def other_players(player)
    other_players_list = []
    @players.each_value do |other_player|
      other_players_list.push(other_player) if player != other_player
    end
    other_players_list
  end
  ...
  def start
    ...
    players.each_value do |player|
      @players.length < 4 ? deal_cards(7, player) : deal_cards(5, player)
    end
    @started = true
  end
  ...
  def play_round(player_name, rank)
    ...
    get_catch = player.hand[rank] ? handle_player_has_card(player, rank) : handle_go_fish(rank)
  ...
  def winner
    @winning_books = 0
    if deck.out_of_cards? || players_out_of_cards
      players.each_value { |player| calculate_winner(player) }
    end
    @winner
  end
  ...
  def handle_player_has_card(player, rank)
  ...
  def handle_go_fish(rank)
    ...
    @hash_to_return = { turn: @turn.name, rank_asked_for: rank, card_from: 'pool', cards: [card] }
  ...
  def calculate_winner(player)
    ...
    elsif player.books == winning_books && player.books.positive?
      @winner.class == Array ? @winner.push(player) : @winner = [@winner, player]
    end
  end

  def create_display(player, cards, rank)
    @hash_to_return = { turn: turn.name, rank_asked_for: rank, card_from: player.name, cards: [] }
